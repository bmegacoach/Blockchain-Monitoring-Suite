...
import { GUARDIAN_ABI, USDGBMINTING_ABI } from './guardianAbi';
...
// ========================
// USDGBMinting Contract Hooks
// ========================

let cachedUSDGBAddress = null;

export const useUSDGBMintingContract = () => {
  const [contract, setContract] = useState(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  ...
  useEffect(() => {
    const initContract = async () => {
      try {
        ...
        // Note: You'll need to add USDGB_MINTING_ADDRESS to your .env.local
        const usdgbAddress = process.env.NEXT_PUBLIC_USDGB_MINTING_ADDRESS || cachedUSDGBAddress;
        if (!usdgbAddress) {
          console.warn('USDGB Minting contract address not configured');
          setError('USDGB Minting address not set');
          return;
        }
        
        cachedUSDGBAddress = usdgbAddress;
        const provider = new ethers.JsonRpcProvider(config.RPC_URL);
        const usdgbContract = new ethers.Contract(
          usdgbAddress,
          USDGBMINTING_ABI,
          provider
        );
        setContract(usdgbContract);
      } catch (err) {
        setError('Failed to initialize USDGB Minting contract');
        console.error('USDGB contract initialization error:', err);
      }
    };
...
export const useComplianceQueue = () => {
  ...
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const { contract } = useUSDGBMintingContract();
  ...
  const fetchQueueItems = useCallback(async () => {
    ...
    setLoading(true);
    try {
      // This is a simplified version - you'll need to implement event listening
      // in a real scenario for real-time updates
      setQueueItems([]);
      setError('');
    } catch (err) {
      setError('Failed to fetch compliance queue');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  return { queueItems, loading, error, refetch: fetchQueueItems };
};

export const useExecuteUSDGBFunction = () => {
  const [transactionHash, setTransactionHash] = useState(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  ...
  const execute = useCallback(async (signer, functionName, ...args) => {
    ...
    try {
      const usdgbAddress = process.env.NEXT_PUBLIC_USDGB_MINTING_ADDRESS || cachedUSDGBAddress;
      if (!usdgbAddress) {
        throw new Error('USDGB Minting contract address not configured');
      }
      ...
      if (functionName === 'approveTransaction') {
        const [txId] = args;
        tx = await usdgbContract.approveTransaction(txId);
      } else if (functionName === 'rejectTransaction') {
        const [txId, reason] = args;
        tx = await usdgbContract.rejectTransaction(txId, reason);
      } else if (functionName === 'addMinter') {
        const [partnerAddress] = args;
        tx = await usdgbContract.addMinter(partnerAddress);
      } else if (functionName === 'removeMinter') {
        const [partnerAddress] = args;
        tx = await usdgbContract.removeMinter(partnerAddress);
      } else {
        throw new Error(`Unknown function: ${functionName}`);
      }
